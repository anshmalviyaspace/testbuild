...
function Marker({ marker, radius, defaultSize, onClick, onHover }: MarkerProps) {
  ...
  return (
    <group ref={groupRef} visible={isVisible}>
      <group position={lineCenter.toArray()} quaternion={lineQuaternion}>
        <mesh>
          <cylinderGeometry args={[0.003, 0.003, lineHeight, 8]} />
          <meshBasicMaterial color="#ffffff" transparent opacity={hovered ? 0.8 : 0.4} />
        </mesh>
      </group>
      <mesh position={surfacePosition.toArray()}>
        <sphereGeometry args={[0.02, 16, 16]} />
        <meshBasicMaterial color="#ffffff" transparent opacity={0.8} />
      </mesh>
      <group ref={imageGroupRef} position={topPosition.toArray()}>
        <Html center distanceFactor={8} style={{ pointerEvents: "auto" }}>
          <div
            className={cn(
              "rounded-full overflow-hidden border-2 border-white/30 cursor-pointer transition-transform duration-200 shadow-lg",
              hovered && "scale-125 border-white/60"
            )}
  ...
  return (
    <group ref={groupRef}>
      <mesh geometry={geometry}>
        <meshStandardMaterial map={earthTexture} bumpMap={bumpTexture} bumpScale={config.bumpScale} metalness={0.1} roughness={0.8} />
      </mesh>
  ...
  return (
    <Html center>
      <div className="flex items-center justify-center">
        <span className="text-sm text-muted-foreground font-mono animate-pulse">Loading globe...</span>
      </div>
    </Html>
  );
}
